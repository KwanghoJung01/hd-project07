...
//   2) 대시보드 → Edge Functions → ai-vision → Secrets 에
//        GEMINI_API_KEY = <Google AI Studio 키>
//        (선택) GEMINI_MODEL = gemini-3.6-flash   ← 비우면 이 값이 기본
...
Deno.serve(async (req: Request) => {
  ...
  if (!key) return json({ error: { message: "서버에 GEMINI_API_KEY 가 설정되지 않았습니다.", status: 500 } }, 500);
